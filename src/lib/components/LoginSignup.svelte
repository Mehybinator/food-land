...
<dialog bind:this={modal} class="modal">
  <div class="modal-box min-w-96 max-w-96 md:max-w-[32rem]">
    <form method="dialog">
      <button class="btn btn-sm btn-circle btn-ghost absolute left-2 top-2 {loading ? 'btn-disabled' : ''}"
        >✕</button
      >
    </form>
    <div class="flex flex-col justify-between items-center h-[32rem]">
      ...
      <div class="flex flex-col place-self-start">
        <div class="flex gap-2 w-2/3">
          <button
            class="font-extrabold text-lg flex-1 btn btn-lg btn-outline {loading ? 'btn-disabled' : 'btn-primary'} !text-base-content hover:!text-primary-content rounded-b-none border-2 border-white {!signup
              ? 'border-b-primary'
          ...
          >
          <button
            class="font-extrabold text-lg whitespace-nowrap flex-1 btn btn-lg btn-outline {loading ? 'btn-disabled' : 'btn-primary'} !text-base-content hover:!text-primary-content rounded-b-none border-2 border-white {signup
              ? 'border-b-primary'
              : 'border-b-base-200'} py-4"
            on:click={() => {
              signup = true;
      ...
      <button class="btn w-full h-16 {loading ? 'btn-disabled' : 'btn-primary'}" on:click={loginSignup}>
        {#if loading}
